  ...
  for (let i = 1; i <= maxPages; i++) {
    res = await fetch(
        `https://api.github.com/users/${
            username}/repos?&sort=pushed&per_page=100&page=${i}`
        // {
        //     headers: {
        //         Accept: 'application/vnd.github+json',
        //         Authorization:
...
const displayRepos = (repos) => {
  ...
  for (const repo of repos) {
    ...
    if (repo.stargazers_count > 0) {
      listItem.innerHTML += `<a href="${starsUrl}" target="_blank">
            <span>⭐ ${repo.stargazers_count}</span></a>`;
    }

    if (repo.language) {
      listItem.innerHTML += `<a href="${langUrl}" target="_blank">
    ...
    if (repo.homepage && repo.homepage !== '') {
      listItem.innerHTML += `<br /> <br />
            <a class="link-btn" href=${repo.html_url}>Code ${
          devicons['Github']}</a>
            <a class="link-btn" href=${repo.homepage}>Live ${
          devicons['Chrome']}</a> <br />`;
    } else {
      listItem.innerHTML += `<br /> <br />
            <a class="link-btn" 
      href="${repo.html_url}" target="_blank">View Project ${
          devicons['Github']}</a><br />`;
    }
...
const devicons = {
  ...
  Java :
      '<i class="devicon-java-plain colored" style="color: #ffca2c"></i> Java',
  ...
  Lua :
      '<i class="devicon-lua-plain-wordmark colored" style="color: #0000d0"></i> Lua',
  Matlab : '<i class="devicon-matlab-plain colored"></i> Matlab',
  Nim :
      '<i class="devicon-nixos-plain colored" style="color: #FFC200"></i> Nim',
  ...
  Rust :
      '<i class="devicon-rust-plain colored" style="color: #DEA584"></i> Rust',
